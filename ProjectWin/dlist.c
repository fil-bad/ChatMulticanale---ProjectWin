...
int add_head_dlist(dlist_pp head, dlist_p node) {
	if (!node) {    //nodo non nullo
		fprintf(stderr, "[dlist]node is NULL!\n");
		return -1;
	}

	if (!head) {    //puntatore del puntatore al nodo testa non nullo
		fprintf(stderr, "[dlist]head is NULL!\n");
		return -1;
	}
...
void *get_head_dlist(dlist_pp head) {
	if (!head || !*head) {
		fprintf(stderr, "[dlist]head or first node is NULL!\n");
		return NULL;
	}
...
int delete_head_dlist(dlist_pp head) {
	...
	if (!head || !*head) {
		fprintf(stderr, "[dlist]No nodes to delete!\n");
		return -1;
	}
