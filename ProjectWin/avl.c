...
int delete_avl_nodes(avl_p root) {
	...
	if (!root) {
		fprintf(stderr, "[avl]root invalid.\n");
		return 0;
	}
...
bool insert_avl_node(avl_pp head, int key, int data) {
	...
	if (!head) {
		fprintf(stderr, "[avl]Initialize AVL tree first\n");
		return FALSE;
	}
	...
	while (root) {
		...
		if (key < root->keyNode) {
			...
			n->node = root;
			if (!n->node) {
				fprintf(stderr, "[insert_avl_node] in stack push node NULL!!!\n");
				Sleep(1);
			}
			n->direction = LEFT;
			push(stack, n);
			...
			if (!root->right) {     //se right è nullo aggiungo lì
				...
				while ((p = pop(stack)) != NULL) {
					if (!p->node) {
						fprintf(stderr, "[insert_avl_node] in stack pop p.node is NULL!!!\n");
						Sleep(1);
					}
					if (!modified) {
						modified = rebalance(stack, head, p->node, key);
...
bool delete_avl_node(avl_pp head, int key) {
	...
	if (!head) {
		printf ( "Initialize AVL tree first\n");
		return FALSE;
	}

	node = *head;
	if (!node) {
		fprintf(stderr, "[avl]No nodes to delete\n");
		return FALSE;
	}
...
int destroy_avl(avl_pp head) {
	...
	if (!head) {
		fprintf(stderr, "[avl]head invalid.\n");
		return -1;
	}
