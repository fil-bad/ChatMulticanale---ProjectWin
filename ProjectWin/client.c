...
connection *initSocket(UINT16 port, char *IP) {
	...
	// METTERE PRIMA SE NO NON FUNZIONA
	WSADATA   wsaData;
...
int keepAlive(SOCKET *ds_sock) {
	...
	char *optval = malloc(sizeof(DWORD));
	// Attiviamo il keepalive
	memset(optval, 1, sizeof(DWORD));
	int ret = setsockopt(*ds_sock, SOL_SOCKET, SO_KEEPALIVE, optval, sizeof(DWORD));
	if (ret == SOCKET_ERROR) {
...
int readPack(SOCKET ds_sock, mail *pack)
{
	SetConsoleCtrlHandler(NULL, TRUE); // disattivo il CTRL+C
	...
	do {
		ret = recv(ds_sock, &mailCRead.md + bRead, sizeof(metadataChar) - bRead, 0);
		if (ret == SOCKET_ERROR) {
			int errNum = GetLastError();
			char buf[256];
			FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
				NULL, errNum, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
				buf, 256, NULL);
			fprintf(stderr, "[READPACK]\n%s\nError n.%d", buf, errNum);
			return -1;
		}
		if (ret == 0) {
			iterContr++;
			if (iterContr > 2) {
				fprintf(stderr, "Seems Read can't go further; test connection...\n");
				if (testConnection(ds_sock) == -1) {
					//pthread_sigmask(SIG_SETMASK, &oldSet, &newSet);   //restora tutto
					SetConsoleCtrlHandler(NULL, FALSE); // riattivo il CTRL+C
					return -1;
				}
			}
		}
	...
	*/
	...
	iterContr = 0;
	do {
		ret = recv(ds_sock, pack->mex + bRead, dimMex - bRead,0);
		if (ret == SOCKET_ERROR) {
			int errNum = GetLastError();
			char buf[256];
			FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
...
int writePack(SOCKET ds_sock, mail pack) //dentro il thArg deve essere puntato un mail
{
	SetConsoleCtrlHandler(NULL, TRUE); // disattivo il CTRL+C
	...
	*/
	...
	do {
		ret = send(ds_sock, &mailCWrite + bWrite, sizeof(metadataChar) - bWrite, 0);
		if (ret == SOCKET_ERROR) {
			int errRet = WSAGetLastError();
			fprintf(stderr, "Error n: %d", errRet);
			return -1;
		}
		bWrite += ret;
	...
	do {
		ret = send(ds_sock, mailCWrite.mex + bWrite, dimMex - bWrite, 0);
		if (ret == SOCKET_ERROR) {
			int errRet = WSAGetLastError();
			fprintf(stderr, "Error n: %d", errRet);
			return -1;
...
{
	int ret = connect(c->ds_sock, (struct sockaddr *) &c->sock, sizeof(c->sock));
	if (ret == SOCKET_ERROR)
	{
		int errNum = GetLastError();
		char buf[256];
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
			NULL, errNum, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
			buf, 256, NULL);
		fprintf(stderr, "[TESTCONNECTION]\n%s\nError n.%d", buf, errNum);
		closesocket(c->ds_sock);
		return -1;
	}
	return 0;
}
...
int createChat(SOCKET ds_sock, mail *pack, table *tabChats) {
...
int deleteChat(SOCKET ds_sock, mail *pack, table *tabChats) {
...
int leaveChat(SOCKET ds_sock, mail *pack, table *tabChats) {
...
int joinChat(SOCKET ds_sock, mail *pack, table *tabChats) {
...
int openChat(SOCKET ds_sock, mail *pack, table *tabChats) {
